...
def _list_types_items() -> list[dict[str, Any]]:
    ...
    for ordinal in range(1, qty + 1):
        try:
            name = ida_typeinf.get_numbered_type_name(_get_idati(), ordinal)  # type: ignore[attr-defined]
        except Exception:
            name = None
        if not name:
            continue
        ...
        try:
            ...
            ida_typeinf.get_numbered_type(_get_idati(), ordinal, tif)  # type: ignore[attr-defined]
            try:
...
def _normalize_entry_point_item(item: dict[str, Any]) -> dict[str, Any]:
    return {
        "name": item.get("name"),
        "address": _as_hex_addr(item.get("ea")),
        "ordinal": item.get("ordinal"),
    }
...
def function_disasm_resource(addr: str) -> str:
    ...
    return _resource_detail(
        "function_disasm",
        address=_as_hex_addr(result.get("start_ea")),
        name=result.get("name"),
        end_address=_as_hex_addr(result.get("end_ea")),
